...
sudo apt-get -q install -y build-essential linux-headers-`uname -r` libnuma-dev python python3 mg

# Download DPDK version 17.08.1 and extract archive
...
MACHINE_CFLAGS=" -march=nehalem" make config T=x86_64-native-linuxapp-gcc
make
MACHINE_CFLAGS=" -march=nehalem" make install T=x86_64-native-linuxapp-gcc
